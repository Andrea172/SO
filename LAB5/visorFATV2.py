...
class App(tk.Tk):
    ...
    def choose_file(self):
        ...
        with open(self.fname,mode='rb') as file:
                ...
                x="a "  
                x = "".join(map(chr,oem))
                self.canvas.create_text(20,40, text="SystemId: "+x,anchor=tk.W)
                print(x.find("FAT"))
                if (x.find("FAT")):
                        file.seek(82)
                        oem=file.read(8) 
                        ...
                        if(x.find("FAT")):    
                            self.fname=None
                            msg = "No se ha elegido la imagen del sistema de archivos correcto"
    ...
    def show_fat(self):
        ...
        if self.fname == None:
            ...
            self.comboBox = ttk.Combobox(self,values=self.clustersRoot)
            self.comboBox.current(1)
            self.comboBox.bind("<<ComboboxSelected>>",self.pintarCluster)
            self.comboBox.grid(row=3,column=1, padx=30)
            
            with open(self.fname,mode='rb') as file:              
                file.seek(self.sectoresReservados * self.bytesInSector)
                ## Lee solo las primeras 512 entradas de la FAT
                fat = []
                for i in range(int(self.sectorInFat*self.bytesInSector/2)):
                    byte=file.read(self.type)
                ...
                for j in range(0,h,30):
                    for i in range(0,w,30):
                        ...
                        cont += 1
                        if cont == self.sectorInFat*self.bytesInSector/2:
                            return
